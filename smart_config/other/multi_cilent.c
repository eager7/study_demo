...
int main(int argc, char *argv[])
{
	...
	WifiEasyConnect(1, &bSSID_data, sizeof(bSSID_data)-1, &bPwd_data, sizeof(bPwd_data)-1);
	while(g_MultiCast)
	{
		WifiEasyConnect(0, &bSSID_data, sizeof(bSSID_data)-1, &bPwd_data, sizeof(bPwd_data)-1);
		usleep(40*1000);
		//usleep(100*1000);
	}
	return 0;
